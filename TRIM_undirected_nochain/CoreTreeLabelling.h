...
bool CoreTree::BFS_Judge(int u, vector<int> &distance)
{
	...
	while (!toVisit.empty())
	{
		...
		{
			// cout << "cur:" << cur << ",d_l[cur]:" << d_l[cur] << ",d:" << distance[cur] << ",u:" << u << endl;
			return false;
		}
		for (int i = 0; i < deg[cur]; i++)
		{
	...
	while (true)
	{
		...
		if (BFS_Judge(uid, dis))
		{
			...
			for (int i = 0; i < nodeGroup[lid].size(); ++i)
			{
				...
				if (disLand[lid][id] + lambda - V[id].lb > max_difference && V[id].ub != V[id].lb)
				{
			...
			// cout << "cnt:" << cnt << ",diff:" << max_difference << endl;
			if (max_difference == 0)
				break;
		}
		if (max_id != -1 && disLand[lid][max_id] + lambda <= max_lb)
